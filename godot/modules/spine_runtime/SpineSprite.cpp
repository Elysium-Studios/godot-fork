...
void SpineSprite::_get_property_list(List<PropertyInfo> *p_list) const {
    ...
    for (size_t i=0; i<current_animations.size(); ++i) {
        String prefix = vformat("ca/%d/", (int)i);
        p_list->push_back(PropertyInfo(Variant::NIL, vformat("ID %d", (int)i), PROPERTY_HINT_NONE, prefix, PROPERTY_USAGE_GROUP));
    ...
    if(animation_state.is_valid() && skeleton.is_valid()){
        ...
        for(size_t i=0; i<current_animations.size(); ++i) {
            ...
            if(a.get_type() == Variant::DICTIONARY){
                ...
                if (track_id < 0) {
                    print_line(vformat("track_id at 'ID %d'  can not be less than 0!", (int)i));
                    continue;
                }
